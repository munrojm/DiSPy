...
class IrrepTools:
    ...
    def __init__(self):
        ...
        stokes.pir_data_read(
            pkg_resources.resource_filename('DiSPy', 'PIR_data.txt'))

    def get_irrep_mat(self, stokes_number: int, dimension: int,
                      symmop: SymmOp, k_params=(None, None, None)):
        """
        ...
        # -- Irrep label
        PIR = open(pkg_resources.resource_filename(
            'DiSPy', 'PIR_data.txt'), "r")
        for line in PIR:
            if str(stokes_number) in line:
                label = line[24:32].strip()
        PIR.close()

        return IrrepMat(stokes_number=stokes_number,
                        dimension=dimension,
                        k_params=k_params,
                        symmop=symmop,
                        matrix=matrix,
    ...
    def get_irrep(self, group_data: List[SymmOp], stokes_number: int,
                  dimension: int, k_params=(None, None, None)):
        """
        ...
        for op in group_data:
            ...
            while(any(np.logical_and((mult*tvec) % 1.0 > 0.05,
                                     (mult*tvec) % 1.0 < 0.95))):
                mult += 1.0
            ...
            irrep_matrices.append(self.get_irrep_mat(stokes_number=stokes_number, dimension=dimension,
                                                     k_params=k_params, symmop=std_mat))

        return Irrep(irrep_matrices=irrep_matrices, group_data=group_data,
                     label=irrep_matrices[0].label, stokes_number=stokes_number,
                     dimension=dimension)
    ...
    def possible_irreps(iso_sg):
        ...
        for line in PIR:
            if iso_sg[0:1].upper() + iso_sg[1:].lower() in line:
                out_str = out_str + "Irrep #" + \
                    line[1:7].strip() + ": " + line[24:32].strip()+'\n'
        PIR.close()

        return out_str

    def projection_diag(self, images: List[Structure], symmop_list: List[SymmOp], irrep: Irrep,
        num_unstar: int, a_map: List, basis: List):
        """
        ...
        for i in range(1, numIm-1):
            atoms1 = images[i].frac_coords
            num1 = images[i].frac_coords
                ...
                if basis[j] == 1:
                    for m in range(0, 3):
                        ...
                        for l in range(len(symmop_list)):
                            ...
                            temp_mode[0, t_im, np.nonzero(a_map[l, t_im, j, :])[0][0], :] += \
                                irrep_matrices[l].matrix[0, 0] * \
                                symmop_list[l].rotation_matrix[:, m]
    ...
    def projection_odiag(self, images: List[Structure], symmop_list: List[SymmOp], num_unstar: int,
                         irrep: Irrep, vec: List, index: int, a_map: List, basis: List):
        """
                ...
                if basis[j] == 1 and any([vec[i, j, p] != 0 for p in range(0, 3)]):
                    for m in range(0, 3):
                        ...
                        for l in range(0, len(symmop_list)):
                            ...
                            n_coord = np.dot(
                                symmop_list[l].rotation_matrix, vec[i, j, :])

                            temp_mode[0, t_im, np.nonzero(a_map[l, t_im, j, :])[
                                0][0], :] += irrep_matrices[l].matrix[index, 0]*n_coord
